...
const JobsList = () => {
  ...
  const confirmDelete = async () => {
    try {
      ...
      const token = localStorage.getItem('token');
      await axios.delete(`/api/alumni/jobs/${jobToDelete._id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
          ...
          >
            ...
            <div className="p-6">
              ...
              <p className="text-sm text-gray-500 text-center mb-6">
                This action cannot be undone. The job posting for "{jobToDelete?.title}" at {jobToDelete?.company} will be permanently removed.
              </p>
...
const JobForm = () => {
  ...
  const fetchJobDetails = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await axios.get(`/api/alumni/jobs/${jobId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
